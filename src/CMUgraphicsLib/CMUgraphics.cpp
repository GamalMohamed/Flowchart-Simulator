...
static LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	switch (msg) {
	case WM_CLOSE:
		MessageBox(hwnd, "Uh-oh! This isn't the right way!\nPlease click on Exit icon in the toolbar.", "Error!", MB_ICONERROR);
		return 0;
	case WM_COMMAND:
		switch (wParam) {
		case 1:
			MessageBox(hwnd, "Team Members:\n\n1. Name\n2. Name\n3. Name", "About", MB_ICONINFORMATION);
			break;
		}
		return 0;
	case WM_LBUTTONDOWN:
...
hInstance(GetModuleHandle(0)), iWindowWidth(iWindWidth), iWindowHeight(iWindHeight) {
	...
	hwndWindow = CreateWindow(ccDefWindClassName, ccDefWindTitle, WS_VISIBLE, iWindXPos,
