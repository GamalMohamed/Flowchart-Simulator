...
void Write::Draw(Output* pOut)
{
	if (!zoomed)
		pOut->DrawWrite(position, width, height, Text, Selected);
	else
		pOut->DrawWrite(position, width, height, Text, Selected, true);
...
bool Write::setData(const string &d) 
{
	Data = "";
...
void Write::PrintInfo(Output* pOut) const
{
	pOut->PrintMessage(Text + "   " + comment + "   ID: " + to_string(ID));
}

int Write::Edit(string s)
{
	bool y = setData(s);
...
void Write::Move(bool& D, int& iX, int& iY, int& iXOld, int& iYOld, Connector** ConnList, int ConnCount, Output* pOut)
{
	if (D == false)
	{
...
{
	OutFile << "WRITE" << "  " << ID << "  " << position.x - width / 2 << "  " << position.y << "  " << Data << "  " << '"' << comment << '"' << endl;
}

void Write::Load(ifstream &Infile, Output* pOut)
{
	int x;
	string s;
...
void Write::Simulate(Statement** StatList, int StatCount, map<string, double> &variables, int curr_idx, int &next_idx, bool& zero, Input* pIn, Output* pOut) const
{
	string tmp, data;
...
string Write::GenerateCode(Statement** StatList, int StatCount, int end_idx, int curr_idx, int &next_idx, int level)
{
	string tabs;
	...
	{
		for (int i = 0; i < vars.size(); ++i)
			Code += tabs + "cout << \"" + vars[i] + " = \" << " + vars[i] + " << endl;\n";
	}
...
void Write::zoomIn(Output* pOut, int n, int i)
{
	Point p;
	zoomed = true;
